...
class Arm(gym.Env):
    def __init__(
        ...
        self.action_space = gym.spaces.MultiDiscrete([
            2*self.max_variable_arm_length_unit, 2*self.max_variable_arm_length_unit,
            2*self.max_variable_angle_unit, 2*self.max_variable_angle_unit,
        ])
    ...
    def update_parameters(self, action: dict[str, int]) -> None:
        self.arm_1_length += self.min_arm_zoom_length*(action[0] - self.max_variable_arm_length_unit)
        self.arm_2_length += self.min_arm_zoom_length*(action[1] - self.max_variable_arm_length_unit)
        self.angle_1_degree += self.min_angle_degree*(action[2] - self.max_variable_angle_unit)
        self.angle_2_degree += self.min_angle_degree*(action[3] - self.max_variable_angle_unit)
    ...
    def step(self, action: dict[str, int]):
        ...
        reward = calculate_reward(
            ...
            position=self.position, target=self.target, max_arm_2_length=self.arm_2_range[0], 
